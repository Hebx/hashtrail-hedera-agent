...
function read(input: EnvInput, key: string, includeProcessEnv: boolean): string | undefined {
  return input[key] ?? (includeProcessEnv ? process.env[key] : undefined);
}

function requireLiveValue(input: EnvInput, key: string, includeProcessEnv: boolean): string {
...
export function loadEnv(input: EnvInput = process.env): HashTrailEnv {
  ...
  const mode = read(input, "HBL_LIVE", includeProcessEnv) === "1" ? "live" : "mock";
  const llmProvider = read(input, "HBL_LLM_PROVIDER", includeProcessEnv) ?? "openai";
  ...
  return {
    ...
    hederaMirrorNodeUrl: read(input, "HEDERA_MIRROR_NODE_URL", includeProcessEnv),
    llmProvider,
    llmModel: read(input, "HBL_LLM_MODEL", includeProcessEnv) ?? "gpt-4o-mini",
    openAiApiKey: read(input, "OPENAI_API_KEY", includeProcessEnv),
    displayName: read(input, "HASHTRAIL_DISPLAY_NAME", includeProcessEnv) ?? "ihab",
